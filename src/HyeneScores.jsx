...
export default function HyeneScores() {
  ...
  // Données Classement
  const [selectedChampionship, setSelectedChampionship] = useState('france');
  const [selectedSeason, setSelectedSeason] = useState('9');

  const championships = [
    { id: 'france', name: 'France', icon: '🇫🇷' },
    { id: 'spain', name: 'Espagne', icon: '🇪🇸' },
    { id: 'italy', name: 'Italie', icon: '🇮🇹' },
    { id: 'england', name: 'Angleterre', icon: '🏴󠁧󠁢󠁥󠁮󠁧󠁿' }
  ];

  const teams = [
    { rank: 1, name: 'MILAN AC', pts: 95, record: '30-5-3', goalDiff: '89:40', diff: '+49' },
    { rank: 2, name: 'STOCKY FC', pts: 88, record: '28-4-6', goalDiff: '82:38', diff: '+44' },
    { rank: 3, name: 'BIMBAM', pts: 82, record: '26-4-8', goalDiff: '76:42', diff: '+34' },
    { rank: 4, name: 'DYNAMO KEV', pts: 75, record: '23-6-9', goalDiff: '71:45', diff: '+26' },
    { rank: 5, name: 'TRAKNAR FC', pts: 68, record: '20-8-10', goalDiff: '65:48', diff: '+17' },
    { rank: 6, name: 'FC GRINTA', pts: 62, record: '18-8-12', goalDiff: '58:51', diff: '+7' },
    { rank: 7, name: 'COMARDINHO', pts: 55, record: '16-7-15', goalDiff: '52:58', diff: '-6' },
    { rank: 8, name: 'MAMBA TEAM', pts: 48, record: '13-9-16', goalDiff: '45:62', diff: '-17' },
    { rank: 9, name: 'WARNAQUE', pts: 35, record: '9-8-21', goalDiff: '38:75', diff: '-37' },
    { rank: 10, name: 'NOPIGOAL FC', pts: 28, record: '7-7-24', goalDiff: '32:89', diff: '-57' }
  ];
  ...
  const getAvailableTeams = (currentMatchId, currentType) => {
    ...
    matches.forEach(match => {
      if (match.id === currentMatchId) {
        if (currentType === 'home' && match.awayTeam) {
          selectedTeams.push(match.awayTeam);
        } else if (currentType === 'away' && match.homeTeam) {
          selectedTeams.push(match.homeTeam);
        }
      } else {
  ...
  const handleTeamSelect = (matchId, type, team) => {
    setMatches(matches.map(m => 
      m.id === matchId 
        ? { ...m, [type === 'home' ? 'homeTeam' : 'awayTeam']: team }
        : m
    ));
  ...
  const toggleDropdown = (matchId, type) => {
    setOpenDropdown(openDropdown?.matchId === matchId && openDropdown?.type === type 
      ? null 
      : { matchId, type }
    );
  };
                        ...
                        <div className="flex-1 min-w-0 relative">
                          ...
                          {openDropdown?.matchId === match.id && openDropdown?.type === 'home' && (
                            <>
                              ...
                              <div className="absolute left-0 right-0 top-full mt-1 bg-gray-900 border border-cyan-500/30 rounded-lg shadow-2xl z-50">
                                <button
